...
const ArraySet = struct {
    ...
    pub fn insert(self: *ArraySet, gpa: Allocator, item: u32) usize {
        ...
        var upper_bound: usize = self.items.len;

        while (lower_bound < upper_bound) {
            const midpoint = lower_bound + (upper_bound - lower_bound) / 2;

            const value_at_midpoint = self.items[midpoint];

            if (item == value_at_midpoint) {
                // No duplicate inserts
                return self.items.len;
            } else if (value_at_midpoint < item) {
    ...
    pub fn delete(self: *ArraySet, value: u32) ?usize {
        const i = self.search(value) orelse return null;
    ...
    fn search(self: *ArraySet, value: u32) ?usize {
        ...
        var lower_bound: usize = 0;
        var upper_bound: usize = self.items.len - 1;
